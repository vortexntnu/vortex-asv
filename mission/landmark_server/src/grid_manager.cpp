...
bool GridManager::point_in_polygon(
    int x, int y, const Eigen::Array<float, 2, Eigen::Dynamic> &polygon) {
  int i, j;
  bool c = false;
  for (i = 0, j = polygon.cols() - 1; i < polygon.cols(); j = i++) {
    if (((polygon(1, i) > y) != (polygon(1, j) > y)) &&
        (x < (polygon(0, j) - polygon(0, i)) * (y - polygon(1, i)) /
                     (polygon(1, j) - polygon(1, i)) +
                 polygon(0, i)))
      c = !c;
  }
  return c;
