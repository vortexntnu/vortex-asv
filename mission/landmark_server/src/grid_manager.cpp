...
namespace landmark_server
{
    ...
    // GridManager::GridManager(std::vector<int8_t> grid, float resolution, uint32_t height, uint32_t width)
    ...
    //     }
    //     grid_ = Eigen::Map<const Eigen::Array<int8_t, Eigen::Dynamic, Eigen::Dynamic>>
    ...
    const GridManager::Grid& GridManager::get_grid() const {
        return grid_;
    }
    
    void GridManager::update_grid(int8_t* grid, const Eigen::Array<float, 2, Eigen::Dynamic>& polygon, int value){
    ...
    //         handle_circle(shape_info.circle.x_centre, shape_info.circle.y_centre, shape_info.circle.radius, value);
    //         break;
...
void GridManager::draw_circle(int xc, int yc, int x, int y, int value, Eigen::Block<landmark_server::GridManager::Grid, -1, -1, false>& block, int xmin, int ymin) {
    // Translate coordinates to block local coordinates
    int bx = xc - xmin;
    int by = yc - ymin;

    if (bx + x < block.rows() && by + y < block.cols() && bx + x >= 0 && by + y >= 0) {
...
void GridManager::handle_polygon(int8_t* grid, const Eigen::Array<float, 2, Eigen::Dynamic>& vertices, int value) {
...
void GridManager::draw_polygon(int x0, int y0, int x1, int y1, Eigen::Block<landmark_server::GridManager::Grid, -1, -1, false>& block, int value) {
...
void GridManager::draw_line(int x0, int y0, int x1, int y1, int8_t* grid, int value) {
    bool steep = std::abs(y1 - y0) > std::abs(x1 - x0);
    if (steep) {
        std::swap(x0, y0);
        std::swap(x1, y1);
