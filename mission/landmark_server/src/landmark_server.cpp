...
void LandmarkServerNode::landmarksRecievedCallback(
    ...
    if (msg->landmarks.empty()) {
        RCLCPP_WARN_STREAM_THROTTLE(this->get_logger(), *this->get_clock(),
                                    5000, "Received empty landmark array");
        return;
    }
    ...
    for (const auto& landmark : msg->landmarks) {
        if (landmark.action == 0) {
            ...
            auto it = std::find_if(storedLandmarks_->landmarks.begin(),
                                   storedLandmarks_->landmarks.end(),
                                   [&](const auto& storedLandmark) {
                                       return storedLandmark.landmark_type ==
                                                  landmark.landmark_type &&
...
geometry_msgs::msg::PoseArray LandmarkServerNode::poseArrayCreater(
    ...
    if (!landmarks.landmarks.empty()) {
        poseArray.header.frame_id =
            landmarks.landmarks.at(0).odom.header.frame_id;
    } else {
...
double LandmarkServerNode::calculateDistance(
    ...
    try {
        ...
        geometry_msgs::msg::TransformStamped transform_stamped =
            tf2_buffer_->lookupTransform("base_link", header.frame_id,
