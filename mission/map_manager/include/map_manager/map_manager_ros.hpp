...
private:
    ...
    pcl::PointCloud<pcl::PointXYZ> processCoordinates(const std::vector<std::array<double, 2>>& coordinates);
    pcl::PointCloud<pcl::PointXYZ> readPolygonFromFile(const std::string& filename);
    ...
    nav_msgs::msg::OccupancyGrid insert_landmask(nav_msgs::msg::OccupancyGrid& grid, const pcl::PointCloud<pcl::PointXYZ>& polygon);
    void handle_get_map_request(
        const std::shared_ptr<rmw_request_id_t> request_header,
        const std::shared_ptr<nav_msgs::srv::GetMap::Request> request,
        const std::shared_ptr<nav_msgs::srv::GetMap::Response> response);
    void fillOutsidePolygon(nav_msgs::msg::OccupancyGrid& grid, const pcl::PointCloud<pcl::PointXYZ>& polygon);
