...
MapManagerNode::MapManagerNode(const rclcpp::NodeOptions &options)
    ...
    //     auto qos_sensor_data = rclcpp::QoS(rclcpp::QoSInitialization(qos_profile.history, 1), qos_profile);
        ...
        "map_origin", qos_transient_local,std::bind(&MapManagerNode::mapOriginCallback, this, std::placeholders::_1));

        declare_parameter("use_predef_landmask", false);
        declare_parameter("landmask_file", "src/vortex-asv/mission/map_manager/params/land_polygon.yaml");
        ...
        map_pub_ = this->create_publisher<nav_msgs::msg::OccupancyGrid>("map", qos_transient_local);
        map_pub_->publish(grid);

        grid_service_ = this->create_service<nav_msgs::srv::GetMap>(
            "get_map", std::bind(
        &MapManagerNode::handle_get_map_request,   // Callback function
    ...
    void MapManagerNode::mapOriginCallback(const sensor_msgs::msg::NavSatFix::SharedPtr msg)
    {
    map_origin_lat_ = msg->latitude;
    map_origin_lon_ = msg->longitude;
    map_origin_set_ = true;
    // Optionally, you can deactivate the subscriber if it should only receive one message
        ...
        const std::shared_ptr<nav_msgs::srv::GetMap::Response> response)
    {
        ...
        auto polygon = readPolygonFromFile(landmask_file_);
        grid = insert_landmask(grid, polygon);
        response->map = grid;
    ...
    pcl::PointCloud<pcl::PointXYZ> MapManagerNode::readPolygonFromFile(const std::string& filename) {
    ...
    pcl::PointCloud<pcl::PointXYZ> MapManagerNode::processCoordinates(const std::vector<std::array<double, 2>>& coordinates)
     {
    ...
    geometry_msgs::msg::Pose MapManagerNode::calculate_map_origin() {
        ...
        double half_width_meters = -(get_parameter("map_width").as_int() * get_parameter("map_resolution").as_double()) / 2.0;
        double half_height_meters = -(get_parameter("map_height").as_int() * get_parameter("map_resolution").as_double()) / 2.0;
        auto [lat, lon] = flat2lla(half_width_meters, half_height_meters);
        geometry_msgs::msg::Pose map_origin;
        map_origin.position.x = lat;
        map_origin.position.y = lon;
...
void drawLine(int x0, int y0, int x1, int y1, nav_msgs::msg::OccupancyGrid& grid, int value) {
...
bool isPointInsidePolygon(int px, int py, const pcl::PointCloud<pcl::PointXYZ>& polygon, const nav_msgs::msg::OccupancyGrid& grid) {
    int count = 0;
    int n = polygon.size();
    for (int i = 0; i < n; i++) {
        const pcl::PointXYZ& start = polygon[i];
        const pcl::PointXYZ& end = polygon[(i + 1) % n];
        ...
        // Check if the line from start to end intersects with the line from (px, py) to (infinity, py)
        if ((y0 > py) != (y1 > py)) {  // If the point is between the y bounds of the segment
            // Calculate x coordinate of the intersection point of the line segment with the horizontal line through py
            float intersectX = (y1 - y0 != 0) ? (x1 - x0) * (float)(py - y0) / (float)(y1 - y0) + x0 : x0;
...
void MapManagerNode::fillOutsidePolygon(nav_msgs::msg::OccupancyGrid& grid, const pcl::PointCloud<pcl::PointXYZ>& polygon) {
...
nav_msgs::msg::OccupancyGrid MapManagerNode::insert_landmask(nav_msgs::msg::OccupancyGrid& grid, const pcl::PointCloud<pcl::PointXYZ>& polygon) 
    {
    int value = 100; // Set this to the desired occupancy value for land (0-100)
    for (uint i = 0; i < polygon.size(); i++) {
        
        // if(i > 1){break;}
        const pcl::PointXYZ& current = polygon[i];
        const pcl::PointXYZ& next = polygon[(i + 1) % polygon.size()]; // Loop back to the first point
        // std::cout << "current: " << current.x << " " << current.y << " next: " << next.x << " " << next.y << "\n";
