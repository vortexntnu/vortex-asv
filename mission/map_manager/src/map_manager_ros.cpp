...
MapManagerNode::MapManagerNode(const rclcpp::NodeOptions &options)
    ...
    //     auto qos_sensor_data = rclcpp::QoS(rclcpp::QoSInitialization(qos_profile.history, 1), qos_profile);
        ...
        landmask_pub_ = this->create_publisher<geometry_msgs::msg::PolygonStamped>("landmask", qos_transient_local);
        // traffic centre
    ...
    geometry_msgs::msg::PolygonStamped MapManagerNode::readPolygonFromFile(const std::string& filename) {
    ...
    geometry_msgs::msg::PolygonStamped MapManagerNode::processCoordinates(const std::vector<std::array<double, 2>>& coordinates)
     {
...
bool isPointInsidePolygon(int px, int py, const geometry_msgs::msg::PolygonStamped& polygon, const nav_msgs::msg::OccupancyGrid& grid) {
    int count = 0;
    int n = polygon.polygon.points.size();
    for (int i = 0; i < n; i++) {
        const geometry_msgs::msg::Point32& start = polygon.polygon.points[i];
        const geometry_msgs::msg::Point32& end = polygon.polygon.points[(i + 1) % n];
...
void MapManagerNode::fillOutsidePolygon(nav_msgs::msg::OccupancyGrid& grid, const geometry_msgs::msg::PolygonStamped& polygon) {
...
void MapManagerNode::insert_landmask(nav_msgs::msg::OccupancyGrid& grid, const geometry_msgs::msg::PolygonStamped& polygon) 
    {
    ...
    for (uint i = 0; i < polygon.polygon.points.size(); i++) {
        ...
        const geometry_msgs::msg::Point32& next = polygon.polygon.points[(i + 1) % polygon.polygon.points.size()]; // Loop back to the first point
