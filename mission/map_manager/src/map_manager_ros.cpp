...
void MapManagerNode::mapOriginCallback(
  ...
  map_origin_set_ = true;
  // Optionally, you can deactivate the subscriber if it should only receive one
  // message
...
pcl::PointCloud<pcl::PointXYZ> MapManagerNode::processCoordinates(
  ...
  for (const auto &coord : coordinates) {
    ...
    pcl::PointXYZ point((float)flat[0], (float)flat[1],
                        0); // Assuming z-coordinate is 0
...
bool isPointInsidePolygon(int px, int py,
                          const pcl::PointCloud<pcl::PointXYZ> &polygon,
...
MapManagerNode::insert_landmask(nav_msgs::msg::OccupancyGrid &grid,
                                const pcl::PointCloud<pcl::PointXYZ> &polygon) {
  ...
  for (uint i = 0; i < polygon.size(); i++) {
    ...
    const pcl::PointXYZ &next =
        polygon[(i + 1) % polygon.size()]; // Loop back to the first point
