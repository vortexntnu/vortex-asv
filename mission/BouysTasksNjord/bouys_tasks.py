...
class DetectedObjectsNavigation():
    ...
    def execute(self, userdata):
        ...
        while self.closest_object != '': # and self.enabled == True:
            ...
            if self.closest_object[1] == 'red' and self.second_closest_object[1] == 'green' or self.closest_object[1] == 'green' and self.second_closest_object[1] == 'red':
                GreenBouy = self.data.current_green_bouy
                RedBouy = self.data.current_red_bouy
                midpoint = ((GreenBouy[0] + RedBouy[0])/2, (GreenBouy[1] + RedBouy[1])/2)
                self.send_wp(self.data.vessel_position)
                self.overwrite_with_new_waypoint(midpoint)

                if self.third_object[1] == 'red' and self.fourth_closest_object[1] == 'green' or self.third_object[1] == 'green' and self.fourth_closest_object[1] == 'red':
                ...
                if self.third_object[1] == 'red':
                  next_waypoint = self.NavAroundOneObject(self.data.vessel_position, self.data.next_red_bouy, self.DistanceRadius, self.DirectionWithLeia)
                  self.send_wp(next_waypoint)

                if self.third_object[1] == 'green':
                  next_waypoint = self.NavAroundOneObject(self.data.vessel_position, self.data.next_green_bouy, self.DistanceRadius, self.DirectionWithLeia)
                  self.send_wp(next_waypoint)


            elif self.closest_object[1] == 'red':
                next_waypoint = self.NavAroundOneObject(self.data.vessel_position, self.data.current_red_bouy, self.DistanceRadius, self.DirectionWithLeia)
                self.send_wp(self.data.vessel_position)
                ...
                next_waypoint = self.NavAroundOneObject(self.data.vessel_position, self.data.current_green_bouy, self.DistanceRadius, self.DirectionWithLeia)
