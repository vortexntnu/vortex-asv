...
DockingTaskNode::DockingTaskNode(const rclcpp::NodeOptions &options)
  ...
  map_origin_sub_ = this->create_subscription<sensor_msgs::msg::NavSatFix>(
      ...
      [this](const sensor_msgs::msg::NavSatFix::SharedPtr msg) {
        ...
        this->set_parameter(
            rclcpp::Parameter("map_origin_lon", msg->longitude));
        this->set_parameter(rclcpp::Parameter("map_origin_set", true));
        RCLCPP_INFO(this->get_logger(), "Map origin set to: %f, %f",
  ...
  grid_sub_ = this->create_subscription<nav_msgs::msg::OccupancyGrid>(
      get_parameter("grid_topic").as_string(), qos_sensor_data,
  ...
  landmarks_sub_ = this->create_subscription<vortex_msgs::msg::LandmarkArray>(
      ...
      [this](const vortex_msgs::msg::LandmarkArray::SharedPtr msg) {
...
void DockingTaskNode::main_task() {
  ...
  RCLCPP_INFO(this->get_logger(),
              "Waiting for system to initialize before starting main task");
...
std::pair<LandmarkWithID, LandmarkWithID> DockingTaskNode::initial_waypoint() {
  ...
  while (true) {
    ...
    if (!(this->get_parameter("map_origin_set").as_bool())) {
      RCLCPP_INFO(this->get_logger(),
                  "Map origin not set, exiting initial waypoint");
      continue;
    }
    ...
    while (result < 10) {
      ...
      try {
        auto transform =
            tf_buffer_->lookupTransform("base_link", "map", rclcpp::Time(0));
        for (size_t i = 0; i < landmark_poses_base_link_frame.poses.size();
             ++i) {
          tf2::doTransform(landmark_poses_map_frame.poses.at(i),
                           landmark_poses_base_link_frame.poses.at(i),
                           transform);
          landmarks_with_ids[i].pose_base_link_frame =
              landmark_poses_base_link_frame.poses[i];
        }
      ...
      for (size_t i = 0; i < landmarks_with_ids.size() - 1; ++i) {
        ...
        // the desired distance range Base link is NED frame, x is forward, y is
        // right
        if ((landmark1.pose_base_link_frame.position.x > 0 &&
          ...
          double x_waypoint_sample = (landmark1.pose_map_frame.position.x +
                                      landmark2.pose_map_frame.position.x) /
                                     2;
          double y_waypoint_sample = (landmark1.pose_map_frame.position.y +
                                      landmark2.pose_map_frame.position.y) /
                                     2;
          std::pair<uint32_t, uint32_t> id_pair_sample = {landmark1.id,
                                                          landmark2.id};
    ...
    try {
      auto transform =
          tf_buffer_->lookupTransform("odom", "map", rclcpp::Time(0));
...
DockingTaskNode::predict_buoy_formation(const LandmarkWithID &buoy1,
                                        const LandmarkWithID &buoy2) const {
  ...
  Eigen::Vector2d direction_vector(
      previous_waypoint_odom_frame_.x -
  ...
  for (const auto &landmark : landmarks_msg_->landmarks) {
    if (landmark.id == buoy1.id &&
        std::sqrt(std::pow(landmark.odom.pose.pose.position.x -
                               buoy1.pose_map_frame.position.x,
                           2) +
                  std::pow(landmark.odom.pose.pose.position.y -
                               buoy1.pose_map_frame.position.y,
                           2)) < 0.5) {
      found++;
    ...
    if (landmark.id == buoy2.id &&
        std::sqrt(std::pow(landmark.odom.pose.pose.position.x -
                               buoy2.pose_map_frame.position.x,
                           2) +
                  std::pow(landmark.odom.pose.pose.position.y -
                               buoy2.pose_map_frame.position.y,
                           2)) < 0.5) {
      found++;
...
Eigen::MatrixXd DockingTaskNode::generate_reward_matrix(
    const Eigen::Array<double, 2, 6> &predicted_positions,
    const Eigen::MatrixXd &measured_positions) {
  ...
  for (int i = 0; i < num_measured; ++i) {
    for (int j = 0; j < num_predicted; ++j) {
      ...
      reward_matrix(i, j) = -distance; // We use negative distance as reward
                                       // (shorter distance = higher reward)
    }
  }
...
Eigen::VectorXi
DockingTaskNode::auction_algorithm(const Eigen::MatrixXd &reward_matrix) {
  ...
  while (true) {
    ...
    try {
      ...
      for (size_t i = 0; i < landmark_poses_map_frame.poses.size(); ++i) {
        tf2::doTransform(landmark_poses_map_frame.poses.at(i),
                         landmark_poses_odom_frame.poses.at(i), transform);
      }
    } catch (tf2::TransformException &ex) {
      RCLCPP_ERROR(this->get_logger(), "Transform error: %s", ex.what());
    }
    ...
    Eigen::MatrixXd reward_matrix =
        generate_reward_matrix(predicted_positions, measured_positions);
...
std::pair<double, double> DockingTaskNode::lla2flat(double lat,
                                                    double lon) const {
...
double DockingTaskNode::get_freya_heading(
