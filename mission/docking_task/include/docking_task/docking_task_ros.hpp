...
public:
   ...
   */
  Eigen::MatrixXd
  generate_reward_matrix(const Eigen::Array<double, 2, 6> &predicted_positions,
  ...
  /**
   * @brief Utility function to get the heading of the ASV assuming identical
   * orientation between odom and map frame
   *
   * @param msg The quat message derived from the odom message published by the
   * seapath
   */
  double get_freya_heading(const geometry_msgs::msg::Quaternion msg) const;

  /**
   * @brief Utility function to set the position of the GPS coordinates in the
   * map frame
   */
