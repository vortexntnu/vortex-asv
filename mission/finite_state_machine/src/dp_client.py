...
def change_control_mode_client(requested_mode):
    ...
    try:
        control_mode = rospy.ServiceProxy("/controllers/control_mode_service", ControlMode)
...
def dp_move_base(x, y, yaw):
    ...
    #action client
    action_client = actionlib.SimpleActionClient("/controllers/move_base", MoveBaseAction)
    ...
    rospy.loginfo("action client made successfully")
    return action_client.get_result()

if __name__ == '__main__':
    try:
        rospy.init_node('dp_move_base')
        change_control_mode_client(HEADING_HOLD)
        dp_move_base(0.0,0.0,1)     
        rospy.spin()
