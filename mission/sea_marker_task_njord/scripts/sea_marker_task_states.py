...
class Maneuvering1(smach.State):
    ...
    def __init__(self, userdata):
        smach.State.__init__(
            self,
            outcomes=['maneuvering1', 'maneuvering2', 'maneuvering3'
                      'stop'],
    ...
    def set_wp_to_avoid_objects(self):
        ...
        for sea_marker in self.sea_marker_list:
            ...
            if (bouy_distance > radius3):
                ...
                tangent_points = calculate_tangent_points(
                    sea_marker[0], sea_marker[1], radius2, position[0],
                    position[1])
                distance_pos_to_tangent_pos = calculate_distance(
                    position, tangent_points[0])
                if distance_pos_to_tangent_pos < distance_tolerance:
                    tangent_points = calculate_tangent_points(
                        sea_marker[0], sea_marker[1], radius1, position[0],
                        position[1])
