...
void ManeuveringTaskNode::main_task() {
  ...
  // Second buoy pair
  Eigen::Array<double, 2, 4> predicted_second_buoy_pair =
  ...
  std::vector<LandmarkPoseID> measured_buoy_0_to_3 =
      get_buoy_landmarks(predicted_second_buoy_pair);
  geometry_msgs::msg::Point waypoint_second_pair;
  waypoint_second_pair.x =
      (measured_buoy_0_to_3[2].pose_odom_frame.position.x +
       measured_buoy_0_to_3[3].pose_odom_frame.position.x) /
      2;
  waypoint_second_pair.y =
      (measured_buoy_0_to_3[2].pose_odom_frame.position.y +
       measured_buoy_0_to_3[3].pose_odom_frame.position.y) /
      2;
  ...
  direction_vector_forwards
      << (measured_buoy_0_to_3[2].pose_odom_frame.position.x +
          measured_buoy_0_to_3[3].pose_odom_frame.position.x) /
                 2 -
             (measured_buoy_0_to_3[0].pose_odom_frame.position.x +
              measured_buoy_0_to_3[1].pose_odom_frame.position.x) /
                 2,
      (measured_buoy_0_to_3[2].pose_odom_frame.position.y +
       measured_buoy_0_to_3[3].pose_odom_frame.position.y) /
              2 -
          (measured_buoy_0_to_3[0].pose_odom_frame.position.y +
           measured_buoy_0_to_3[1].pose_odom_frame.position.y) /
              2;
  ...
  geometry_msgs::msg::Point red_buoy =
      measured_buoy_0_to_3[2].pose_odom_frame.position;
  geometry_msgs::msg::Point green_buoy =
      measured_buoy_0_to_3[3].pose_odom_frame.position;
  ...
  for (int _ = 0; _ < num_iterations; ++_) {
    ...
    waypoint_next_pair.x = (measured_next_pair[2].pose_odom_frame.position.x +
                            measured_next_pair[3].pose_odom_frame.position.x) /
                           2;
    waypoint_next_pair.y = (measured_next_pair[2].pose_odom_frame.position.y +
                            measured_next_pair[3].pose_odom_frame.position.y) /
                           2;
    ...
    vector_to_next_pair
        << (measured_next_pair[2].pose_odom_frame.position.x +
            measured_next_pair[3].pose_odom_frame.position.x) /
                   2 -
               (measured_next_pair[1].pose_odom_frame.position.x +
                measured_next_pair[0].pose_odom_frame.position.x) /
                   2,
        (measured_next_pair[2].pose_odom_frame.position.y +
         measured_next_pair[3].pose_odom_frame.position.y) /
                2 -
            (measured_next_pair[1].pose_odom_frame.position.y +
             measured_next_pair[0].pose_odom_frame.position.y) /
                2;
  }
  ...
  if (distance_to_end > 6.0) {
    ...
    waypoint_toward_end.x =
        ...
        direction_vector_to_end(0) * (distance_to_end - 3.0);
    waypoint_toward_end.y =
        odom->pose.pose.position.y +
        direction_vector_to_end(1) * (distance_to_end - 3.0);
...
Eigen::Array<double, 2, 4> ManeuveringTaskNode::predict_second_buoy_pair(
