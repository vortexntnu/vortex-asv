...
NjordTaskBaseNode::NjordTaskBaseNode(const std::string &node_name, const rclcpp::NodeOptions &options)
...
void NjordTaskBaseNode::setup_map_odom_tf_and_subs()
{
  ...
  while (!tf_set) {
  try {
    auto transform = tf_buffer_->lookupTransform("odom", "map", tf2::TimePointZero, tf2::durationFromSec(1.0));
  ...
  odom_sub_ = this->create_subscription<nav_msgs::msg::Odometry>(
      ...
      std::bind(&NjordTaskBaseNode::odom_callback, this, std::placeholders::_1));
  ...
  landmarks_sub_ = this->create_subscription<vortex_msgs::msg::LandmarkArray>(
      ...
      std::bind(&NjordTaskBaseNode::landmark_callback, this, std::placeholders::_1));
}
...
std::pair<double, double> NjordTaskBaseNode::lla2flat(double lat, double lon) const
{
...
void NjordTaskBaseNode::map_origin_callback(const sensor_msgs::msg::NavSatFix::SharedPtr msg)
{
...
void NjordTaskBaseNode::odom_callback(const nav_msgs::msg::Odometry::SharedPtr msg)
{
...
void NjordTaskBaseNode::landmark_callback(const vortex_msgs::msg::LandmarkArray::SharedPtr msg)
{
  std::unique_lock<std::mutex> lock(landmark_mutex_);
  // transform the landmarks to the odom frame
  for (auto &landmark : msg->landmarks) {
    tf2::doTransform(landmark.odom.pose.pose, landmark.odom.pose.pose, map_odom_tf_);
  }
  landmarks_msg_ = msg;
...
std::shared_ptr<vortex_msgs::msg::LandmarkArray> NjordTaskBaseNode::get_landmarks_odom_frame()
{
  std::unique_lock<std::mutex> lock(landmark_mutex_);
...
Eigen::VectorXi NjordTaskBaseNode::assign_landmarks(const Eigen::Array<double, 2, Eigen::Dynamic> &predicted_positions,
  ...
  // Extract the final assignment for real predicted positions, ignoring dummy items
