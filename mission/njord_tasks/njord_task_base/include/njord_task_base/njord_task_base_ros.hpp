...
{
public:
  NjordTaskBaseNode(const std::string &node_name, const rclcpp::NodeOptions &options);
...
protected:
  ...
  /**
   * @brief Assign landmarks to buoys based on the predicted and measured positions using the auction algorithm.
   *
   ...
   * where each element is the index of the measured position(landmark) assigned to the corresponding predicted position(buoy).
   * If an element is -1, it means that the corresponding predicted position(buoy) is not assigned to any measured position(landmark).
   */
  Eigen::VectorXi assign_landmarks(const Eigen::Array<double, 2, Eigen::Dynamic> &predicted_positions,
