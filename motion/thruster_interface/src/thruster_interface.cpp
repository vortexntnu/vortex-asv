...
void send_pwm(std::vector<uint16_t> pwm_values, int file){
    ...
    if(static_cast<size_t>(write(file, bytes.data(), bytes.size())) != bytes.size()){
...
void get_pwm_table() {
  // Open the data file
  std::ifstream file("src/vortex-asv/motion/thruster_interface/config/ThrustMe_P1000_force_mapping.csv");
  if (!file.is_open()) {
