...
void ThrusterInterfaceROS::thrustCallback(
  ...
  for (int i = 0; i < 4; i++) { // Iterates through thruster 0 to 3, where 0 is front right, iterated clockwise
    ...
    int offset_from_center_value = pwm_values[i] - center_pwm_value + pwm_offsets[i];
    int pwm_value_correct_direction = center_pwm_value + thruster_direction_map[i] * offset_from_center_value;

    int pwm_clamped =
        std::min(std::max(pwm_value_correct_direction + pwm_offsets[i], 1100), 1900);
