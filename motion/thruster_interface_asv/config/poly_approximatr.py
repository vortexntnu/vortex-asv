...
def read_csv_and_plot(filename):
    ...
    x_fit_negative = np.linspace(
        negative_data['Force'].min(), negative_data['Force'].max(), 100
    )
    ...
    # Plot original data
    plt.scatter(
        df['Force'], df['PWM'], color='blue', s=20, alpha=0.5, label='All Data Points'
    )
    ...
    plt.scatter(
        ...
        s=20,
        alpha=0.7,
        label='Points Used for Fitting (Force > 0)',
    )
    ...
    # Plot polynomial fit
    plt.plot(x_fit, y_fit, 'r-', linewidth=2, label=f'3rd Order Polynomial Fit\n{eq}')
...
# Example usage
filename = "motion/thruster_interface_asv/config/ThrustMe_P1000_force_mapping.csv"
