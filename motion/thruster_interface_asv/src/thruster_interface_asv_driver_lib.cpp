...
std::vector<_ForcePWM> _loadDataFromCSV(const std::string& filepath) {
    ...
    // Try opening the file and if succeeded fill our datastructure with all the
    ...
    if (file.is_open()) {
        ...
        while (std::getline(file, line)) {
            ...
            // Define data structure format we want our .CSV values to be
            // ordered in
            _ForcePWM ForcePWMDataStructure;

            // Data manipulation ----------
            // csvLineSplit variable converts "line" variable to a char stream
            // of data that can further be used to split and filter out values
            // we want
            ...
            // Convert grams into Newtons as we expect to get Forces in Newtons
            // but the .CSV file contains forces in grams
            ...
            // Push processed data with correct formatting into the complete
            // dataset
            // ----------
...
int16_t* _interpolate_force_to_pwm(float* forces) {
    ...
    for (int8_t i = 0; i < 4; i++) {
        // Edge case, if the force is out of the bounds of your data table,
        // handle accordingly
        if (forces[i] <= _ForcePWMTable.front().force) {
            interpolatedPWMArray[i] = static_cast<int16_t>(
                _ForcePWMTable.front().pwm);  // Too small Force

        } else if (forces[i] >= _ForcePWMTable.back().force) {
            interpolatedPWMArray[i] = static_cast<int16_t>(
                _ForcePWMTable.back().pwm);  // Too big Force
            ...
            // Run the loop until the force value we are given is lower than
            // the current dataset value
            ...
            // Linear interpolation formula: y = y0 + (y1 - y0) * ((x - x0) /
            // (x1 - x0))
            float interpolatedPWM =
                low.pwm + (high.pwm - low.pwm) * ((forces[i] - low.force) /
...
void _send_pwm_to_ESCs(int16_t* pwm) {
    ...
    for (int8_t i = 0; i < 4; i++) {
        // Split the integer into most significant byte (MSB) and least
...
// The main core functionality of interacting and controlling the thrusters
int16_t* drive_thrusters(float* thrusterForces) {
    ...
    // Send PWM values through I2C to the microcontroller to control thrusters
    _send_pwm_to_ESCs(pwm);

    // Return PWM values for debugging and logging purposes
    return pwm;
}
}  // namespace thruster_interface_asv_driver_lib
