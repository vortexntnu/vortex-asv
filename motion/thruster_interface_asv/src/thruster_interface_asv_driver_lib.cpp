...
namespace thruster_interface_asv_driver_lib {
    ...
    std::vector<_ForcePWM> _loadDataFromCSV(const std::string& filepath) {
        ...
        // Try opening the file and if suceeded fill our datastructure with all the values in the .CSV file
        ...
        if (file.is_open()) {
            ...
            while (std::getline(file, line)) {
                ...
                // Data manipulation ----------
                // csvLineSplit variable converts "line" variable to a char stream of data that can further be used to split and filter out values we want
                ...
                // Convert grams into Newtons as we expect to get Forces in Newtons but the .CSV file calculates forsces in grams
                formatOfForcePWMDatastructure.force = formatOfForcePWMDatastructure.force * (9.81/1000.0);
    ...
    int16_t* _interpolate_force_to_pwm(float* forces) {
        ...
        for (int8_t i = 0; i < 4; i++) {
            // Edge case, if the force is out of the bounds of your data table, handle accordingly
            if (forces[i] <= _datatableForForceAndPWM.front().force) {
                interpolatedPWMArray[i] = static_cast<int16_t>(_datatableForForceAndPWM.front().pwm); // To small Force
            } 
            else if (forces[i] >= _datatableForForceAndPWM.back().force) {
                interpolatedPWMArray[i] = static_cast<int16_t>(_datatableForForceAndPWM.back().pwm); // To big Force
            } 
            else {
                ...
                // Run the loop until the force value we are givven is lower than the current dataset value
                ...
                float interpolatedPWM = low.pwm + (high.pwm - low.pwm) * ((forces[i] - low.force) / (high.force - low.force));
    ...
    // The most important function that sends PWM values through I2C to Arduino that then sends PWM signal further to the ESCs
    ...
    void init(const std::string& pathToCSVFile, int8_t* thrusterMapping, int8_t* thrusterDirection, int16_t* offsetPWM, int16_t* minPWM, int16_t* maxPWM) {
        ...
        // We load it here instead of interpolation step as this will save time as we only open and load all the data once, savind time in intrepolation isnce we dont need to open the .CSV file over and over again.
    ...
    int16_t* drive_thrusters(float* thrusterForces) {
        ...
        for (int8_t pinNr = 0; pinNr < 4; pinNr++) {
            ...
            thrusterForcesCahngedMapping[pinNr] = thrusterForces[remapedThrusterForcesIndex];
        }

        // Change direction of the thruster (Forward(+1)/Backwards(-1)) according to the direction parameter
        float thrusterForcesCahngedDirection[4] = {0.0, 0.0, 0.0, 0.0};
        for (int8_t i = 0; i < 4; i++) {
            thrusterForcesCahngedDirection[i] = thrusterForcesCahngedMapping[i] * _thrusterDirection[i];
        }
