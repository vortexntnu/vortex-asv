...
class ThrusterInterfaceASVNode : public rclcpp::Node {
  private:
    ...
    rclcpp::Subscription<std_msgs::msg::Float32MultiArray>::SharedPtr _subscriberThrusterForces;
    ...
    // Methods ----------
    void _thruster_forces_callback(const std_msgs::msg::Float32MultiArray::SharedPtr msg) const {
    ...
    void _thruster_driver_callback() {
      ...
      std::shared_ptr<std_msgs::msg::Int16MultiArray> messagePWM = std::make_shared<std_msgs::msg::Int16MultiArray>();
  ...
  public:
    ...
    ThrusterInterfaceASVNode() : Node("thruster_interface_asv_node") {
      ...
      std::vector<int64_t> parameterThrusterMapping = this->declare_parameter<std::vector<int64_t>>("propulsion.thrusters.thruster_to_pin_mapping", {0, 1, 2, 3});
      ...
      std::vector<int64_t> parameterThrusterDirection = this->declare_parameter<std::vector<int64_t>>("propulsion.thrusters.thruster_direction", {1, 1, 1, 1});
      ...
      std::vector<int64_t> parameterPWMOffset = this->declare_parameter<std::vector<int64_t>>("propulsion.thrusters.thruster_PWM_offset", {0, 0, 0, 0});
      int16_t offsetPWM[4] = {
        static_cast<int16_t>(parameterPWMOffset[0]),
      ...
      std::vector<int64_t> parameterPWMMin = this->declare_parameter<std::vector<int64_t>>("propulsion.thrusters.thruster_PWM_min", {1100, 1100, 1100, 1100});
      std::vector<int64_t> parameterPWMMax = this->declare_parameter<std::vector<int64_t>>("propulsion.thrusters.thruster_PWM_max", {1900, 1900, 1900, 1900});
      int16_t minPWM[4] = {
        static_cast<int16_t>(parameterPWMMin[0]),
        static_cast<int16_t>(parameterPWMMin[1]),
        static_cast<int16_t>(parameterPWMMin[2]),
        static_cast<int16_t>(parameterPWMMin[3])
        };
      int16_t maxPWM[4] = {
        static_cast<int16_t>(parameterPWMMax[0]),
      ...
      _subscriberThrusterForces = this->create_subscription<std_msgs::msg::Float32MultiArray>(
        "/thrust/thruster_forces", 
        5, 
        std::bind(&ThrusterInterfaceASVNode::_thruster_forces_callback, this, std::placeholders::_1)
        );
      ...
      // Initialize a never ending cycle that continuisly publishes and drives thrusters depending on what the ThrusterForces value is set to from ThrusterForces subscriber
      ...
      // Debugging ----------
      RCLCPP_INFO(this->get_logger(), "Initialized thruster_interface_asv_node node");
    }
};
