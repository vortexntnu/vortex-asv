...
private:
   ...
   *
   * @param tau_openloop            A 6d torque vector from the open loop
   * control
   *
   * @param position_state          A 3d vector containing the current body
   * position
   ...
   * setpoint
   *
   * @return  A feedback torque vector for maintaining both a fixed pose and
   * heading
   */
  Eigen::Vector6d
  poseHeadingHold(const Eigen::Vector6d &tau_openloop,
                  const Eigen::Vector3d &position_state,
                  const Eigen::Quaterniond &orientation_state,
                  const Eigen::Vector6d &velocity_state,
                  const Eigen::Vector3d &position_setpoint,
