...
/**
 * @brief a class containing the current state data for position, orientation
 * and velocity
 */
class State {
public:
   ...
   *
   * @param position        A pointer whos value will be set to the current
   * position state
   * @param orientation     A pointer whos value will be set to the current
   * orientation state
   *
   ...
   * @param orientation     A pointer whos value will be set to the current
   * orientation state
   * @param velocity        A pointer whos value will be set to the current
   * velocity state
   *
   * @return false if states are not initialized, true otherwise
   */
   ...
   *
   * @param orientation     A pointer whos value will be set to the euler
   * orientation
   *
   * @return false if states are not initialized, true otherwise
   */
   ...
   *
   * @warning All state values are considered invalid until this function is
   * called.
   */
...
private:
  ...
  Eigen::Quaterniond
      m_orientation; /** A quaternion containing the orientation state*/
  Eigen::Vector6d m_velocity; /** A 6d vector containing the velocity state*/

  bool m_is_initialized; /** A bool to indicate whether or not the states are
                            initialized. False before set() is called*/
};
