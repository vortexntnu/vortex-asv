...
QuaternionPdController::QuaternionPdController(double a, double b, double c,
                                               double i, double W, double B,
...
Eigen::Vector6d
QuaternionPdController::getRestoring(const Eigen::Quaterniond &q) {
  // Rotate from inertial/world to body
  Eigen::Matrix3d R = q.toRotationMatrix();
  return restoringForceVector(R);
}

Eigen::Vector6d QuaternionPdController::getFeedback(
    const Eigen::Vector3d &x, const Eigen::Quaterniond &q,
    const Eigen::Vector6d &nu, const Eigen::Vector3d &x_d,
...
Eigen::Matrix6d
QuaternionPdController::proportionalGainMatrix(const Eigen::Matrix3d R) {
...
Eigen::Matrix6d
QuaternionPdController::integralGainMatrix(const Eigen::Matrix3d R) {
  return (Eigen::Matrix6d() << R.transpose() * m_K_i,
          Eigen::MatrixXd::Zero(3, 3), Eigen::MatrixXd::Zero(3, 3),
          m_c_i * Eigen::MatrixXd::Identity(3, 3))
      .finished();
}

void QuaternionPdController::integralWindUp(Eigen::Vector6d &vec,
                                            double pose_lim, double att_lim) {
...
Eigen::Vector6d QuaternionPdController::errorVector(
...
Eigen::Vector6d
QuaternionPdController::restoringForceVector(const Eigen::Matrix3d R) {
...
Eigen::Vector3d
QuaternionPdController::referenceModel(const Eigen::Vector3d &x,
                                       const Eigen::Vector3d &x_ref) {

  Eigen::Vector3d x_d;

  Eigen::Vector3d a_x(1, -1.990024937655860, 0.990049813123053);
  Eigen::Vector3d b_x(6.218866798092052e-06, 1.243773359618410e-05,
                      6.218866798092052e-06);
