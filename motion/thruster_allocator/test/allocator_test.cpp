...
class AllocatorTest : public testing::Test, public rclcpp::Node {
    ...
    public:
      AllocatorTest() : Node("AllocatorTest_node") {
        subscription_ = this->create_subscription<vortex_msgs::msg::ThrusterForces>("thrust/thruster_forces", 1,
          std::bind(&AllocatorTest::thruster_forces_callback, this, std::placeholders::_1));
    ...
    private:
      ...
      void thruster_forces_callback(const vortex_msgs::msg::ThrusterForces &thruster_msg) {
      ...
      bool isNodeReady() {
        return (count_subscribers("thrust/thruster_forces") > 0) && (count_publishers("thrust/wrench_input") > 0);
      }

};


TEST_F(AllocatorTest, DumbTest) {
  ASSERT_EQ(1, 1);
}
