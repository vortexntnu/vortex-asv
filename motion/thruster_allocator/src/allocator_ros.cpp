...
Allocator::Allocator(): Node("thrust_allocator_node"){
    ...
    subscription_ = this->create_subscription<geometry_msgs::msg::Wrench>(
        "thrust/wrench_input", 1, std::bind(&Allocator::wrench_callback, this, std::placeholders::_1));

    publisher_ = this->create_publisher<vortex_msgs::msg::ThrusterForces>("thrust/thruster_forces", 1);
...
void Allocator::spinOnce(){
    ...
    //u vector
    Eigen::VectorXd thruster_forces = m_pseudoinverse_allocator->calculateAllocatedThrust(body_frame_forces);
...
void Allocator::wrench_callback(const geometry_msgs::msg::Wrench &msg) {
    //const Eigen::VectorXd body_frame_forces = wrenchMsgToEigen(msg); // Wrench health check
...
Eigen::VectorXd Allocator::wrenchMsgToEigen(const geometry_msgs::msg::Wrench &msg) const {
...
Eigen::VectorXd Allocator::wrenchMsgToEigen(const float force_x, const float force_y, const float torque) const {
