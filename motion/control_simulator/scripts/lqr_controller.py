...
class LQRController:
    ...
    def control(self, state, dt):
        ...
        if not np.array_equal(np.sign(state[:2] - self.setpoint[:2]), np.sign(self.integral_states)): # Integral reset on sign change
            self.integral_states = np.array([0.0, 0.0]) 
    ...
    def linearize(self, state, dt):
        ...
        A[0:3, 3:6] = dt * np.array([
            ...
            [np.sin(state[self.yaw_idx]), np.cos(state[self.yaw_idx]), 0],
            [0, 0, 1]
        ])
        A[6, 0] = dt  # add integral effect on x
        A[7, 1] = dt  # add integral effect on y

        B = np.block([[np.zeros((3, 3))], [dt * np.linalg.inv(self.M)], [np.zeros((2, 3))]])
