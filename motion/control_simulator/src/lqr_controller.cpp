...
public:
  LQRController(Eigen::Matrix3d M, Eigen::Matrix3d D, Eigen::VectorXd Q,
                Eigen::Vector3d R, Vector6d setpoint, double actuator_limits);

  Eigen::Vector3d control(Vector6d state, double dt);

  std::pair<Eigen::MatrixXd, Eigen::MatrixXd> linearize(Vector8d state,
                                                        double dt);

  Eigen::Matrix<double, 3, 8> lqr(Eigen::MatrixXd A, Eigen::MatrixXd B,
                                  Eigen::Matrix<double, 8, 8> Q,
                                  Eigen::Matrix3d R);
};
...
double ssa(double angle) { return fmod((angle + M_PI), (2 * M_PI)) - M_PI; }

LQRController::LQRController(Eigen::Matrix3d M, Eigen::Matrix3d D,
                             Eigen::VectorXd Q, Eigen::Vector3d R,
                             Vector6d setpoint, double actuator_limits) {
  this->Q = Q.asDiagonal();
  this->R = R.asDiagonal();
...
Eigen::Vector3d LQRController::control(Vector6d state, double dt) {
  ...
  std::pair<Eigen::MatrixXd, Eigen::MatrixXd> linearized =
      linearize(state_with_integral, dt);
  ...
  error.segment<6>(0) -=
      setpoint; // Subtract 'setpoint' from the first 6 elements of 'error'
  error.segment<2>(6) -= Eigen::Vector2d(
      0, 0); // Subtract [0, 0] from the last 2 elements of 'error'
  ...
  if ((state.head(2) - setpoint.head(2)).cwiseSign() !=
      integral_states.cwiseSign()) {
...
LQRController::linearize(Vector8d state, double dt) {
  Eigen::MatrixXd A = Eigen::MatrixXd::Identity(8, 8);
  Eigen::Matrix3d Minv = M.inverse();
  A.block<3, 3>(3, 3) -= dt * Minv * D;
  A.block<3, 3>(0, 3) =
      dt * (Eigen::Matrix3d() << std::cos(state[yaw_idx]),
            -std::sin(state[yaw_idx]), 0, std::sin(state[yaw_idx]),
            std::cos(state[yaw_idx]), 0, 0, 0, 1)
               .finished();
...
Eigen::MatrixXd dareSolver(Eigen::MatrixXd A, Eigen::MatrixXd B,
  ...
  for (i = 0; i < max_iterations; i++) {
    X_prev = X;
    X = A.transpose() * X_prev * A -
        A.transpose() * X_prev * B *
            (R + B.transpose() * X_prev * B).inverse() * B.transpose() *
            X_prev * A +
        Q;
  ...
  if (i == max_iterations) {
    std::cout << "Warning: Maximum number of iterations reached in DARE solver."
              << std::endl;
  }
...
Eigen::Matrix<double, 3, 8> LQRController::lqr(Eigen::MatrixXd A,
                                               Eigen::MatrixXd B,
                                               Eigen::Matrix<double, 8, 8> Q,
                                               Eigen::Matrix3d R) {
  Eigen::MatrixXd X = dareSolver(A, B, Q, R);
  Eigen::MatrixXd K =
      (B.transpose() * X * B + R).inverse() * B.transpose() * X * A;
...
int main() {
  ...
  for (int i = 0; i < num_steps; i++) {
    ...
    state.segment<3>(0) +=
        dt * state.segment<3>(3); // Update position based on velocity
    state.segment<3>(3) +=
        dt * acceleration; // Update velocity based on acceleration
