...
class DataLoggerNode:
    def __init__(self):
        ...
        self.odom_sub = rospy.Subscriber('/estimator/pose', Odometry, self.odom_callback)
        self.battery_sub = rospy.Subscriber('/internal/status/bms', BatteryState, self.battery_callback)
        ...
        self.csv_writer.writerow(['latitude', 'longitude', 'altitude', 'heading', 'speed', 'power_consumption', 'battery%', 'timestep'])
    ...
    def find_battery_percentage_left(self, voltage):
        return round((voltage - self.min_voltage) / (self.max_voltage - self.min_voltage) * 100, 2)
        
    def odom_callback(self, data):
        self.log_lock.acquire()
        self.current_odom = [data.pose.pose.position.x, 
                             data.pose.pose.position.y, 
                             data.pose.pose.position.z, 
                             self.quaternion_to_yaw(data.pose.pose.orientation), 
                             np.sqrt(data.twist.twist.linear.x**2 + data.twist.twist.linear.y**2)]
