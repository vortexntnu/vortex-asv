...
The `hybridpath_guidance_node` is responsible for **generating the trajectory** for the ASV to follow.
It receives a list of waypoints and calculates a continuous path between them, publishing desired position and speed references for the controller.
...
`HybridpathGuidance` *(under development)*
...
- ### `seapath/odom/ned`
  ...
  | **Published by**   | [`seapath_ros_driver_node`] (https://github.com/vortexntnu/vortex-seapath-driver/tree/main) |
...
- ### `hybridpath_guidance`
  ...
  Transmits **reference trajectory data** generated by the guidance system.
  The message includes desired *position*, *velocity*, and *acceleration* values, along with control parameters required for **trajectory tracking**.
  This topic connects **high-level navigation planning** with the controller, enabling **smooth and continuous movement** along a mission path.
