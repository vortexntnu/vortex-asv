...
class DStarLite:
    ...
    -------------------------------------------------------------------------------------------
        ...
        get_waypoints() -> list[list[int]]: Retrieves the waypoints and adjusts their coordinates to the original coordinate system.
    ...
    def __init__(
        ...
        goal: Point,
        origin: Point,
        min_dist_to_obstacle: float = 4.5,
        height: int = 25,
        width: int = 25,
    ):
        ...
        if len(obstacles) == 0:  # If no obstacles are provided
            ...
            self.obstacles = [
                DSLNode(int(point.x), int(point.y)) for point in obstacles
            ]  # The obstacles as nodes
    ...
    def is_obstacle(self, dsl_node: DSLNode) -> bool:
        """Check if the DSLNode is considered an obstacle or is too close to an obstacle.

        Args:
            dsl_node (DSLNode): The DSLNode to check.
        ...
        node_xy = np.array([dsl_node.x, dsl_node.y])
    ...
    def calculate_key(self, s: DSLNode) -> tuple:
        ...
        return (
            ...
            + self.heuristic_distance(s)
            + self.key_min,
            min(self.g[s.x][s.y], self.rhs[s.x][s.y]),
        )

    def is_valid(self, dsl_node: DSLNode) -> bool:
        ...
        """
        return (
            self.x_min <= dsl_node.x < self.x_max
            and self.y_min <= dsl_node.y < self.y_max
        )
    ...
    def pred(self, u: DSLNode) -> list[DSLNode]:
        ...
        """
        return [
            u + motion for motion in self.possible_motions if self.is_valid(u + motion)
        ]

    def initialize(self):
        """Initializes the grid and the D* Lite algorithm.

        This function adjusts the coordinates of the start and goal nodes based on
        the grid's minimum world coordinates, sets up the cost and right-hand side (rhs)
        grids, and initializes the priority queue. This setup is required before
        the algorithm begins pathfinding. The initialization will only occur once;
    ...
    def update_vertex(self, u: DSLNode):
        ...
        if not u == self.goal:
            self.rhs[u.x][u.y] = min(
                [
                    self.movement_cost(u, sprime) + self.g[sprime.x][sprime.y]
                    for sprime in self.pred(u)
                ]
            )

        # Update the priority queue
        self.priority_queue = [
            (dslnode, key) for dslnode, key in self.priority_queue if not u == dslnode
        ]
    ...
    def compute_shortest_path(self):
        ...
        while self.priority_queue:
            ...
            start_key_not_updated = has_elements and self.compare_keys(
                self.priority_queue[0][1], start_key
            )
    ...
    def compute_current_path(self) -> list[DSLNode]:
        ...
        while not current_point == self.goal:
            ...
            current_point = min(
                self.pred(current_point),
                key=lambda sprime: self.movement_cost(current_point, sprime)
    ...
    def get_waypoints(self) -> list[Point]:
        ...
        for wp in self.waypoints:
            waypoint_list.append(Point(x=float(wp.x), y=float(wp.y), z=0.0))
